...
def ycsb(voldemort,workload,recordcount):
  ...
  if len(voldemort)>0 :
    ...
    cmd='cd /home/ubuntu/ycsb-0.1.4/ && bin/ycsb run voldemort -p bootstrap_urls=tcp://'+voldemort_ip+':6666 -p recordcount='+recordcount+' -P workloads/'+ workload + ' > res/voldemort.res'
    local(cmd)
  #MongoDB
  # if len(mongo)>0 : 
