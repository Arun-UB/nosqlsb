...
def ycsb(voldemort,workload,recordcount):
  ...
  #Cassandra
  # if len(cassandra)>0 : 
    cmd='cd /home/ubuntu/ycsb-0.1.4/ && bin/ycsb load cassandra-10 -s -P workloads/'+ workload + ' -p recordcount=10000  -p hosts=10.80.77.61 -threads 10 -p recordcount='+recordcount+' > res/cassandra.res'
    local(cmd)
